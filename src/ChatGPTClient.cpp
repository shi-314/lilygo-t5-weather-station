...
const char* OPENAI_API_KEY = "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";
...
void ChatGPTClient::setModel(const String& modelName) {
    model = modelName;
}
...
String ChatGPTClient::generateImage(const String& prompt) {
    ...
    // Create OpenAI DALL-E image generation format
    DynamicJsonDocument doc(4096);
    doc["prompt"] = prompt;
    doc["n"] = 1;
    doc["size"] = "1024x1024";
    doc["response_format"] = "b64_json";
    ...
    DynamicJsonDocument responseDoc(32768); // Larger buffer for image data
    deserializeJson(responseDoc, response);
    
    if (responseDoc.containsKey("data") && 
        responseDoc["data"].size() > 0 &&
        responseDoc["data"][0].containsKey("b64_json")) {
        
        String base64Data = responseDoc["data"][0]["b64_json"].as<String>();
        
        // Decode base64 to binary data
        return base64ToByteArray(base64Data);
    }

    Serial.println("Error: Could not find image data in response");
    Serial.println(response);
    
    return "";
}

String ChatGPTClient::base64ToByteArray(const String& base64String) {
    // Simple approach: return the base64 string as-is for now
    // In a real implementation, you would decode this to binary data
    // For ESP32/Arduino, you might want to save directly to SPIFFS/SD card
    // or use a proper base64 decoding library
    
    Serial.println("Base64 image data received, length: " + String(base64String.length()));
    return base64String;
} 